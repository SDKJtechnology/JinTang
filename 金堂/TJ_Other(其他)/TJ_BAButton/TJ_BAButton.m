...
- (instancetype _Nonnull)initWithFrame:(CGRect)frame
    ...
    if ((self = [TJ_BAButton buttonWithType:UIButtonTypeCustom])) {
        self.frame = frame;
...
@implementation TJ_BACustomButton
...
{
    TJ_BACustomButton *button = [[TJ_BACustomButton alloc] init];
...
- (void)alignmentTop
{
    ...
    self.titleLabel.frame = CGRectMake((self.center.x - frame.size.width) * 0.5, self.imageView.frame.origin.y + BA_imageHeight + 5, frame.size.width, frame.size.height);
...
{
    CGRect frame = self.frame;
    frame.size.width = [self getButtonWidth].size.width;
    self.frame = frame;
}
