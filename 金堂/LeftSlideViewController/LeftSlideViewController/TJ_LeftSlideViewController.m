...
@interface TJ_LeftSlideViewController ()<UIGestureRecognizerDelegate>
{
...
@implementation TJ_LeftSlideViewController
                     ...
                     andMainView:(UIViewController *)mainVC
{
    if(self = [super init]){
        ...
        [self.leftVC.view addGestureRecognizer:self.pan];
...
//    NSLog(@"%f  %f",[rec velocityInView:rec.view].x,[rec velocityInView:rec.view].y);
        ...
        CGFloat scale = 1 - (1 - kMainPageScale) * (self.mainVC.view.frame.origin.x / (kScreenWidth - kMainPageDistance));
        
        self.mainVC.view.transform = CGAffineTransformScale(CGAffineTransformIdentity,scale, scale);
        [rec setTranslation:CGPointMake(0, 0) inView:self.view];
        
        CGFloat leftTabCenterX = kLeftCenterX + ((kScreenWidth - kMainPageDistance) * 0.5 - kLeftCenterX) * (self.mainVC.view.frame.origin.x / (kScreenWidth - kMainPageDistance));
        
//        NSLog(@"%f",leftTabCenterX);
        
        
        //leftScale kLeftScale~1.0
        CGFloat leftScale = kLeftScale + (1 - kLeftScale) * (self.mainVC.view.frame.origin.x / (kScreenWidth - kMainPageDistance));
        ...
        CGFloat tempAlpha = kLeftAlpha - kLeftAlpha * (self.mainVC.view.frame.origin.x / (kScreenWidth - kMainPageDistance));
        self.contentView.alpha = tempAlpha;

    }
    //根据视图位置判断是左滑还是右边滑动
    if (needMoveWithTap && (rec.view.frame.origin.x >= 0) && (rec.view.frame.origin.x <= (kScreenWidth - kMainPageDistance)) && rec.view != self.leftVC.view)
    {
